...
/** 
 * Renders EM Window functionality
 */
appModule.controller('EventMaintenanceController', ['$rootScope', '$scope', '$q', '$route' ,'EventMaintenanceService', function($rootScope, $scope, $q, $route, EventMaintenanceService) {
	...
	$scope.eventMaintenanceWindowSettings = {
        ...
        close: function(event){
            ...
            $("#showRoleDropDownList").jqxDropDownList('selectedIndex', 0); 
            $route.reload();
        }
    };
    ...
    //EM Window has been opened
    $scope.showEMWindow = function(eventId) {
        $scope.eventMaintenanceWindowSettings.apply('open');
        $scope.selectedEvent = eventId;

        //major service calls
        var usersForEvent = EventMaintenanceService.getUsersForEvent(eventId);
        var scriptsForEvent = EventMaintenanceService.getScriptsForEvent(eventId);
        var EMUsers = EventMaintenanceService.getEMUsers(eventId);
        var EMScripts = EventMaintenanceService.getEMScripts(eventId);
